...
require_relative './unsubscribe_by_mail'

module MicrosoftGraph
    module Groups
        module Item
            module UnsubscribeByMail
                ## 
                # Provides operations to call the unsubscribeByMail method.
                class UnsubscribeByMailRequestBuilder < MicrosoftKiotaAbstractions::BaseRequestBuilder
                    ...
                    def initialize(path_parameters, request_adapter)
                        super(path_parameters, request_adapter, "{+baseurl}/groups/{group%2Did}/unsubscribeByMail")
                    end
                    ## 
                    ...
                    ## @return a unsubscribe_by_mail_request_builder
                    ## 
                    def with_url(raw_url)
                        raise StandardError, 'raw_url cannot be null' if raw_url.nil?
                        return UnsubscribeByMailRequestBuilder.new(raw_url, @request_adapter)
                    end
                end
            end
        end
