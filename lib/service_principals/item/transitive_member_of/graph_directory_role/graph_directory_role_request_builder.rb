...
require_relative './graph_directory_role'

module MicrosoftGraph
    module ServicePrincipals
        module Item
            module TransitiveMemberOf
                module GraphDirectoryRole
                    ## 
                    # Casts the previous resource to directoryRole.
                    class GraphDirectoryRoleRequestBuilder < MicrosoftKiotaAbstractions::BaseRequestBuilder
                        
                        ## 
                        # Provides operations to count the resources in the collection.
                        def count()
                            return MicrosoftGraph::ServicePrincipals::Item::TransitiveMemberOf::GraphDirectoryRole::Count::CountRequestBuilder.new(@path_parameters, @request_adapter)
                        end
                        ## 
                        ...
                        def initialize(path_parameters, request_adapter)
                            super(path_parameters, request_adapter, "{+baseurl}/servicePrincipals/{servicePrincipal%2Did}/transitiveMemberOf/graph.directoryRole{?%24top,%24skip,%24search,%24filter,%24count,%24orderby,%24select,%24expand}")
                        end
                        ## 
                        ...
                        ## @return a graph_directory_role_request_builder
                        ## 
                        def with_url(raw_url)
                            raise StandardError, 'raw_url cannot be null' if raw_url.nil?
                            return GraphDirectoryRoleRequestBuilder.new(raw_url, @request_adapter)
                        end
