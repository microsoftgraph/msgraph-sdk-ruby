        ...
        module Item
            module GetMailTips
                ...
                class GetMailTipsRequestBuilder < MicrosoftKiotaAbstractions::BaseRequestBuilder
                    ...
                    ## @return a Fiber of get_mail_tips_post_response
                    ## 
                    def post(body, request_configuration=nil)
                        ...
                        return @request_adapter.send_async(request_info, lambda {|pn| MicrosoftGraph::Users::Item::GetMailTips::GetMailTipsPostResponse.create_from_discriminator_value(pn) }, error_mapping)
                    end
                    ## 
                    ## Get the MailTips of one or more recipients as available to the signed-in user. Note that by making a POST call to the getMailTips action, you can request specific types of MailTips tobe returned for more than one recipient at one time. The requested MailTips are returned in a mailTips collection. This API is available in the following national cloud deployments.
                    ## @param body The request body
                    ...
                    ## @return a get_mail_tips_request_builder
                    ## 
                    def with_url(raw_url)
                        raise StandardError, 'raw_url cannot be null' if raw_url.nil?
                        return GetMailTipsRequestBuilder.new(raw_url, @request_adapter)
                    end
                end
            end
        end
